...
token = "7353194292:AAHpuqWx9IiOi5YSaocbdkeOoyEifgIRkiE" #Your Token BOT
...
def start(message):
    bot.reply_to(message, 'WELCOME TO LAST_WARNING VIDEO DOWNLOADER BOT DOWNLOADING ALL SOCIAL MEDIA VIDEO LIKE :- YOUTUBE ,INSTAGRAM ,FACEBOOK ,TIK TOK, :- TOOL DEVELOPER BY @hardhackar007  .')
...
def Instagram(message):
    bot.reply_to(message, 'PLEASE WAIT LAST WARNING SEARCHING YOUR URL')
    link = message.text
    ...
    if "ok" in res.text:
        ...
        if video_url:
            ...
            markup.add(btn_high)
            bot.send_message(message.chat.id, "@hardhackar007 Choose the video quality:", reply_markup=markup)
    else:
        bot.reply_to(message, "Unsupported URL.")
...
def callback_query(call):
    ...
    if video_url:
        ...
        msg = bot.reply_to(call.message, "@hardhackar007 please wait download is processing")
        ...
        bot.send_video(call.message.chat.id, open(viod, 'rb'), caption="LAST WARNING ⚠️ VIDEO PROVIDE BY:- @hardhackar007")
        os.remove(viod)
    else:
